...
public:
    ...
    void compile(string code)
    {
        ...
        jsCode ~= "const util = require('util');";
        jsCode ~= "(function() {";
...
private:
    ...
    void parseWritef(bool newline)
    {
        ...
        if (newline)
        {
            if (args.length == 0)
            {
                jsCode ~= format("  console.log(%s);", formatStr);
            }
            else
            {
                jsCode ~= format("  console.log(util.format(%s, %s));", 
                               formatStr, args.join(", "));
            }
        }
        else
        {
            if (args.length == 0)
            {
                jsCode ~= format("  process.stdout.write(%s);", formatStr);
            }
            else
            {
                jsCode ~= format("  process.stdout.write(util.format(%s, %s));", 
                               formatStr, args.join(", "));
            }
        }
    }
